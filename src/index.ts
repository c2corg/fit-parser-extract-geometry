...
const log = debug(`fit-parser-extract-geometry`);
...
const invariant = <T>(value: T): NonNullable<T> => {
...
class FIT {
  ...
  private readData(blob: Uint8Array, baseType: number, littleEndian: boolean, startIndex: number): number {
    const { name: type, size } = invariant(BASE_TYPES[baseType]);
  ...
  private isInvalidValue(data: number, baseType: number): boolean {
    const { invalidValue } = invariant(BASE_TYPES[baseType]);
  ...
  private readTimestamp(blob: Uint8Array, startIndex: number, fieldDefinitions: FieldDefinition[]): number | null {
    ...
    for (let i = 0; i < fieldDefinitions.length; i++) {
      const def = invariant(fieldDefinitions[i]);
  ...
  private readDeveloperFieldDefinition(
    ...
    const size = invariant(blob[fieldDefinitionIndex + 1]);
  ...
  private readFieldDefinition(
    ...
    const fieldDefinitionNumber = invariant(blob[fieldDefinitionIndex]);
    /** Base type of the specified message's field (unsigned char, signed short, etc) */
    const baseType = invariant(blob[fieldDefinitionIndex + 2]);
    /** Size (in bytes) of the specified message's field */
    const size = invariant(blob[fieldDefinitionIndex + 1]);
  ...
  private readDefinitionMessage(blob: Uint8Array, startIndex: number): Message {
     ...
     */
    const recordHeader = invariant(blob[startIndex]);
    ...
    /** Architecture type */
    const isLittleEndian = invariant(blob[startIndex + 2]) === 0;
    /** Number of fields in the data messages */
    const numberOfFields = invariant(blob[startIndex + 5]);
    /** Number of Self Descriptive fields in the Data Message */
    const numberOfDeveloperDataFields = hasDeveloperData ? invariant(blob[startIndex + 5 + numberOfFields * 3 + 1]) : 0;
    /** Global message number, unique to each message. Endianness of the 2 bytes value is defined in the architecture byte */
    const globalMessageNumber = this.addEndian(isLittleEndian, [
    ...
    this.messageTypes[localMessageType] = {
      ...
      globalMessageNumber,
      fieldDefinitions: [
        ...new Array(numberOfFields)
          .fill(undefined)
          .map((_, i) => this.readFieldDefinition(blob, startIndex, i, isLittleEndian)),
        ...new Array(numberOfDeveloperDataFields)
          .fill(undefined)
          .map((_, i) => this.readDeveloperFieldDefinition(blob, startIndex, numberOfFields, i)),
      ],
    };
  ...
  extractGeometry(blob: Uint8Array): number[][] {
    if (blob.length < 12) {
     ...
     */
    const headerSize = invariant(blob[0]);
    /** Length of the data records section in bytes. Does not include Header or CRC */
    const dataLength =
      invariant(blob[4]) + (invariant(blob[5]) << 8) + (invariant(blob[6]) << 16) + (invariant(blob[7]) << 24);
    ...
    if (headerSize !== 14 && headerSize !== 12) {
      log(`Incorrect FIT header size: ${headerSize}`);
      return [];
    }
    ...
    const [first, second, ...other] = geometry;
    if (first?.[2] === 0 && second?.[2] !== 0) {
      first[2] = invariant(invariant(second)[2]);
      geometry = [first, invariant(second), ...other];
    }
...
const extractGeometry = (blob: Uint8Array): number[][] => new FIT().extractGeometry(blob);
